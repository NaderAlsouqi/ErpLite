...
import {
  OutgoingCheq1Service,
  OutgoingCheq1LineListItemDto,
  SaveOutgoingCheq1Request,
  SaveOutgoingCheq1Line,
} from '../../../shared/services/outgoing-cheq1.service';
...
@Component({
  ...
  templateUrl: './outgoing-cheq1.component.html',
  styleUrl: './outgoing-cheq1.component.scss',
  encapsulation: ViewEncapsulation.None,
})
export class OutgoingCheq1Component implements OnInit {
  ...
  curNo = 1;
  rate = 1;
  sts = 0;   // 0=في الصندوق, 1=برسم التحصيل, 3=شيكات راجعه
  userName = '';
  ...
  // ─── Status options ──────────────────────────────────────────
  statusOptions = [
    { value: 0, labelKey: 'OutgoingCheq1.StsInBox' },
    { value: 1, labelKey: 'OutgoingCheq1.StsCollection' },
    { value: 3, labelKey: 'OutgoingCheq1.StsReturned' },
  ];

  // ─── List ───────────────────────────────────────────────────
  displayedColumns = ['DocNum', 'Date', 'CheqNum', 'Date1', 'BankName', 'Amt', 'CustAcc', 'Draw', 'Actions'];
  dataSource = new MatTableDataSource<OutgoingCheq1LineListItemDto>([]);
  allListData: OutgoingCheq1LineListItemDto[] = [];
  totalItems = 0;
  pageSize = 50;
  pageIndex = 0;
  ...
  constructor(
    ...
    private cheq1Service: OutgoingCheq1Service,
  ...
  loadLookups(): void {
    ...
    this.voucherSerialService.getAll(13).subscribe({
      next: d => {
  ...
  goPrev(): void {
    this.cheq1Service.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'PREV').subscribe({
      ...
      error: () => this.toastr.info(this.translate.instant('OutgoingCheq1.FirstVoucher'), ''),
    });
  }
  ...
  goNext(): void {
    this.cheq1Service.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'NEXT').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('OutgoingCheq1.LastVoucher'), ''),
    });
  }
  ...
  onCustAccSelected(i: number, acc: ChartOfAccountDto | null): void {
    if (acc) { this.lines[i].custAcc = acc.no; this.lines[i].custAccName = acc.name ?? ''; }
  }
  ...
  save(): void {
    ...
    const req: SaveOutgoingCheq1Request = {
      DocNum:   this.docNum,
      VType:    this.vType,
      MyYear:   this.myYear,
      BrNo:     this.brNo,
      ...
      Lines:    validLines.map<SaveOutgoingCheq1Line>(l => ({
    ...
    this.cheq1Service.save(req).subscribe({
      next: result => {
        this.toastr.success(
          this.translate.instant('OutgoingCheq1.SavedSuccessfully', { docNum: result.DocNum }),
  ...
  private validateForm(): boolean {
    ...
    if (!this.accNo) {
      this.toastr.warning(
        this.translate.instant('OutgoingCheq1.AccNoRequired'),
    ...
    if (validLines.length === 0) {
      this.toastr.warning(
        this.translate.instant('OutgoingCheq1.LinesRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    const missingDraw = validLines.some(l => !l.draw?.trim());
    if (missingDraw) {
      this.toastr.warning(
        this.translate.instant('OutgoingCheq1.DrawRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (duplicate) {
      this.toastr.warning(
        this.translate.instant('OutgoingCheq1.DuplicateCheq'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
  ...
  executeDelete(): void {
    ...
    this.cheq1Service.delete(this.docNum, this.myYear, this.vType).subscribe({
      next: () => {
        this.toastr.success(
          this.translate.instant('OutgoingCheq1.DeletedSuccessfully'),
          this.translate.instant('General.Success')
        );
        this.initNewVoucher();
      },
  ...
  loadList(): void {
    this.loading = true;
    this.cheq1Service.getLinesList(this.myYear, this.vType, this.pageIndex + 1, this.pageSize).subscribe({
      next: res => {
  ...
  onSortChange(sort: Sort): void {
    ...
    this.dataSource.data = data.sort((a, b) => {
      ...
      switch (sort.active) {
        ...
        case 'Amt':    return this.compare(a.Amt, b.Amt, asc);
        default: return 0;
      }
    });
  }

  openVoucherFromList(item: OutgoingCheq1LineListItemDto): void {
    this.activeTab = 'form';
  ...
  printVoucher(): void {
    ...
    const title = this.translate.instant('OutgoingCheq1.Title');

    const cols = [
      { label: this.translate.instant('OutgoingCheq1.CheqNum') },
      { label: this.translate.instant('OutgoingCheq1.Date1') },
      { label: this.translate.instant('OutgoingCheq1.Bank') },
      { label: this.translate.instant('OutgoingCheq1.Amt') },
      { label: this.translate.instant('OutgoingCheq1.CustAcc') },
      { label: this.translate.instant('OutgoingCheq1.Draw') },
    ];
    ...
    const filtersHtml = [
      fi(this.translate.instant('OutgoingCheq1.DocNum'),  String(this.docNum)),
      fi(this.translate.instant('OutgoingCheq1.Year'),    String(this.myYear)),
      fi(this.translate.instant('OutgoingCheq1.Date'),    this.date),
      fi(this.translate.instant('OutgoingCheq1.AccNo'),   `${this.accNo || ''} — ${this.accName}`),
      fi(this.translate.instant('OutgoingCheq1.Status'),  this.translate.instant(this.stsLabel)),
      fi(this.translate.instant('OutgoingCheq1.BrNo'),    String(this.brNo)),
    ].join('');
