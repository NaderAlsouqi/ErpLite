...
@Component({
  selector: 'app-virtual-transfer-refunds',
  standalone: true,
  imports: [
    CommonModule,
    TranslateModule,
...
export class VirtualTransferRefundsComponent implements OnInit {
  // Table configuration
  ...
  private loadData(deliveryId: number): void {
    ...
    this.virtualInvoiceService.GetUntransferredRefundsMainData(deliveryId).subscribe({
      next: (data) => {
      ...
      error: (error: any) => {
        ...
        this.toastr.error(
          this.translate.instant('VirtualTransferRefundsPage.LoadError'),
  ...
  viewRefund(docNumber: string, invoiceNumber: string, year: string): void {
    try {
      this.router.navigate(['/sales/virtual/refund-details'], {
        queryParams: {
          doc: docNumber,
          bill: invoiceNumber,
          year: year
      ...
      this.toastr.error(
        this.translate.instant('VirtualTransferRefundsPage.NavigationError'),
        this.translate.instant('General.Error')
      );
    }
  }
  ...
  async transferSelectedRefunds(): Promise<void> {
    ...
    if (this.selectedDocNumbers.size === 0) {
      this.toastr.warning(
        this.translate.instant('VirtualTransferRefundsPage.NoRefundsSelected'),
    ...
    try {
      ...
      for (const docNumber of this.selectedDocNumbers) {
        try {
          ...
          const invoiceData = await firstValueFrom(
            this.fotaraService.getTransferVirtualRefundInvoiceData(
              docNumber,
          ...
          if (fotaraResponse.status === 'success' && fotaraResponse.qr_code) {
            ...
            await firstValueFrom(
              this.fotaraService.updateVirtualRefundQrCode(
                docNumber,
                refundData.InvoiceNumber,
                refundData.FinancialYear,
      ...
      if (successfulTransfers.length > 0) {
        const successMessage = this.translate.instant('VirtualTransferRefundsPage.SuccessMessage');
        const detailMessage = this.translate.instant('VirtualTransferRefundsPage.TransferSuccessDetail', {
      ...
      if (failedTransfers.length > 0) {
        const warningMessage = this.translate.instant('VirtualTransferRefundsPage.PartialFailure');
        const failureDetail = this.translate.instant('VirtualTransferRefundsPage.TransferFailureDetail', {
      ...
      this.toastr.error(
        this.translate.instant('VirtualTransferRefundsPage.TransferError'),
        this.translate.instant('General.Error')
      );
    } finally {
      // Clear selections since transferred items will no longer be available
      this.selectedDocNumbers.clear();
      this.selectedRefundData.clear();

      // Always reload data to reflect the current state in the database
  ...
  private async refreshData(): Promise<void> {
    ...
    try {
      await new Promise<void>((resolve) => {
        this.virtualInvoiceService.GetUntransferredRefundsMainData(this.deliveryId).subscribe({
          next: (data) => {
            const formattedData = data.map((refund: any) => ({
              InvoiceDate: this.formatDate(refund.InvoiceDate),
              DocumentNumber: refund.DocumentNumber,
          ...
          error: (error: any) => {
            console.error('Error refreshing refunds data:', error);
            this.toastr.error(
              this.translate.instant('VirtualTransferRefundsPage.RefreshError'),
              this.translate.instant('General.Error')
            );
            resolve(); // Resolve despite error to continue execution
          }
        });
      });
    } finally {
