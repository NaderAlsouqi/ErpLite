...
export class InvoiceDetailsComponent implements OnInit {
  ...
  // Table data
  displayedColumns: string[] = ['ItemNumber', 'ItemName', 'Quantity', 'ItemPrice', 'ItemTotalPrice', 'ItemTaxRate', 'ItemTaxAmount', 'ItemTotalPriceAfterTax'];
  ...
  /**
   * Load invoice details from the service
  ...
  private loadInvoiceDetails(TransactionNumber: string): void {
    ...
    this.invoiceService.getInvoiceDetails(TransactionNumber).subscribe({
      next: (data: any) => {
  ...
  /**
   * Print the invoice as a PDF report
   */
  printInvoice(): void {
    this.loading = true;
    
    this.reportService.InvoiceReport(this.TransactionNumber).subscribe({
      next: (response: Blob) => {
