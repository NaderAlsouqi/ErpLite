...
export class QuotationComponent {
  ...
  ngOnInit(): void {
    ...
    if (user) {
      ...
      this.hasRole = (!user.Roles.includes('VirtualSales') && user.Roles.includes('Sales'));      
    }
  }
  ...
  private loadData(deliveryId: number): void {
    ...
    {
      this.InvoiceService.GetQuotationsMainData(deliveryId).subscribe({
      next: (data) => {
      ...
      error: (error) => {
        ...
        this.toastr.error(
          this.translate.instant('Nav.Quotation.LoadError'),
    ...
    {
   this.virtualInvoiceService.GetQuotationsMainData(deliveryId).subscribe({
      next: (data) => {
        console.log('Quotations data:', data);
  ...
  printInvoice(transactionNumber: string, invoiceNumber: string): void {
    ...
    if (this.SystemType==1)
    {
      this.reportService.printQuotation(transactionNumber, invoiceNumber).subscribe({
      next: (response: Blob) => {
    ...
    {
    this.reportService.VirtualprintQuotation(transactionNumber, invoiceNumber).subscribe({
      next: (response: Blob) => {
        const blobUrl = window.URL.createObjectURL(new Blob([response], { type: 'application/pdf' }));
        window.open(blobUrl);
        this.loading = false;
