...
import {
  ChequeCollectionService,
  ChequeCollectionListItemDto,
  ChequeCollectionLineDto,
  SaveChequeCollectionRequest,
  SaveChequeCollectionLine,
  AvailableChequeForCollectionDto,
} from '../../../shared/services/cheque-collection.service';
...
@Component({
  ...
  styleUrl: './cheque-collection.component.scss',
...
export class ChequeCollectionComponent implements OnInit {
  ...
  availableCheques: AvailableChequeForCollectionDto[] = [];
  ...
  dataSource = new MatTableDataSource<ChequeCollectionListItemDto>([]);
  allListData: ChequeCollectionListItemDto[] = [];
  totalItems = 0;
  pageSize = 20;
  pageIndex = 0;
  ...
  rowLinesCache = new Map<string, ChequeCollectionLineDto[]>();
  expandLoadingKey: string | null = null;

  private rowKey(row: ChequeCollectionListItemDto): string {
    return `${row.DocNum}|${row.VType}|${row.MyYear}`;
  }
  toggleRowExpand(row: ChequeCollectionListItemDto): void {
    ...
    if (!this.rowLinesCache.has(key)) {
      ...
      this.collectionService.getVoucher(row.DocNum, row.MyYear, row.VType).subscribe({
        next: v => {
  ...
  isRowExpanded(row: ChequeCollectionListItemDto): boolean { return this.expandedRowKey === this.rowKey(row); }
  isRowLoading(row: ChequeCollectionListItemDto): boolean { return this.expandLoadingKey === this.rowKey(row); }
  getExpandedLines(row: ChequeCollectionListItemDto): ChequeCollectionLineDto[] {
  ...
  saving = false;
  saveAttempted = false;
  isSaved = false;   // true when editing an existing saved document
  ...
  constructor(
    ...
    private collectionService: ChequeCollectionService,
  ...
  loadLookups(): void {
    ...
    this.voucherSerialService.getAll(9).subscribe({
      next: d => {
    ...
    this.collectionService.getAvailableCheques().subscribe({
  ...
  loadDocNums(): void {
    this.collectionService.getList(this.myYear, this.vType, 1, 9999).subscribe({
      next: res => {
  ...
  initNewVoucher(): void {
    this.loading = true;
    this.collectionService.getNextDocNum(this.myYear, this.vType).subscribe({
      next: res => {
  ...
  loadNavigation(): void {
    this.collectionService.getNavigation(this.myYear, this.vType).subscribe({
  ...
  loadVoucher(docNum: number): void {
    this.loading = true;
    this.collectionService.getVoucher(docNum, this.myYear, this.vType).subscribe({
      next: v => {
        const h = v.Header;
        ...
        for (const l of this.lines) {
          if (l.cheqNum && !knownNums.has(l.cheqNum)) {
            ...
            } as AvailableChequeForCollectionDto];
  ...
  goPrev(): void {
    this.collectionService.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'PREV').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('ChequeCollection.FirstVoucher'), ''),
    });
  }

  goNext(): void {
    this.collectionService.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'NEXT').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('ChequeCollection.LastVoucher'), ''),
    });
  }
  ...
  removeLine(i: number): void {
    // Mirrors VB6 delete-line check: saved document cheques always have
    // outstanding deposit movements (InType=1, DocType=6). Block individual
    // line removal and require deleting the entire document instead.
    if (this.isSaved) {
      this.toastr.warning(
        this.translate.instant('ChequeCollection.CannotRemoveLine'),
  ...
  fillLineFromCheque(i: number, cheq: AvailableChequeForCollectionDto): void {
  ...
  save(): void {
    ...
    const req: SaveChequeCollectionRequest = {
      DocNum:   this.docNum,
      VType:    this.vType,
      MyYear:   this.myYear,
      BrNo:     this.brNo,
      ...
      Lines:    this.lines
        ...
        .map<SaveChequeCollectionLine>(l => ({
          CheqNum: l.cheqNum,
          Amt:     l.amt,
          Draw:    l.draw,
          BankNum: l.bankNum ?? 0,
    ...
    this.saving = true;
    this.collectionService.save(req).subscribe({
      next: result => {
        this.toastr.success(
          this.translate.instant('ChequeCollection.SavedSuccessfully', { docNum: result.DocNum }),
  ...
  private validateForm(): boolean {
    if (this.rate < 0) {
      this.toastr.warning(
        this.translate.instant('ChequeCollection.RateNegative'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    if (this.date) {
      ...
      if (y !== this.myYear) {
        this.toastr.warning(
          this.translate.instant('ChequeCollection.DateOutOfYear', { year: this.myYear }),
          this.translate.instant('General.ValidationError')
        );
        return false;
      }
    ...
    if (!this.debitAcc) {
      this.toastr.warning(
        this.translate.instant('ChequeCollection.DebitAccRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    if (!this.creditAcc) {
      this.toastr.warning(
        this.translate.instant('ChequeCollection.CreditAccRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (validLines.length === 0) {
      this.toastr.warning(
        this.translate.instant('ChequeCollection.LinesRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (keys.find((k, i) => keys.indexOf(k) !== i)) {
      this.toastr.warning(
        this.translate.instant('ChequeCollection.DuplicateCheq'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
  ...
  executeDelete(): void {
    this.modalRef?.close();
    this.loading = true;
    this.collectionService.delete(this.docNum, this.myYear, this.vType).subscribe({
      next: () => {
        this.toastr.success(
          this.translate.instant('ChequeCollection.DeletedSuccessfully'),
          this.translate.instant('General.Success')
        );
        this.initNewVoucher();
      },
  ...
  openVoucherFromList(item: ChequeCollectionListItemDto): void {
    this.activeTab = 'form';
  ...
  printVoucher(): void {
    ...
    const title = this.translate.instant('ChequeCollection.Title');
    ...
    const filtersHtml = [
      fi(this.translate.instant('ChequeCollection.DocNum'), String(this.docNum)),
      fi(this.translate.instant('ChequeCollection.Year'),   String(this.myYear)),
      fi(this.translate.instant('ChequeCollection.Date'),   this.date),
      fi(this.translate.instant('ChequeCollection.DebitAcc'),  `${this.debitAcc || ''} — ${this.debitAccName}`),
      fi(this.translate.instant('ChequeCollection.CreditAcc'), `${this.creditAcc || ''} — ${this.creditAccName}`),
    ].join('');

    const cols = [
      { label: this.translate.instant('ChequeCollection.CheqNum') },
      { label: this.translate.instant('ChequeCollection.Amt') },
      { label: this.translate.instant('ChequeCollection.Draw') },
      { label: this.translate.instant('ChequeCollection.VhrNo') },
      { label: this.translate.instant('ChequeCollection.Date1') },
      { label: this.translate.instant('ChequeCollection.BankNum') },
      { label: this.translate.instant('ChequeCollection.Bank') },
    ];
  ...
  // ─── Lookup helpers ─────────────────────────────────────────
  chequeSearchFn(term: string, item: AvailableChequeForCollectionDto): boolean {
    if (!term) return true;
