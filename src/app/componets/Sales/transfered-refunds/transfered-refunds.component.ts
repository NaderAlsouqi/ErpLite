...
@Component({
  selector: 'app-transfered-refunds',
  standalone: true,
  imports: [
    CommonModule,
    TranslateModule,
  ...
  templateUrl: './transfered-refunds.component.html',
  styleUrl: './transfered-refunds.component.scss'
})
export class TransferedRefundsComponent implements OnInit {
  // Table configuration
  ...
  /**
   * Load transferred refunds data from service
   */
  private loadData(deliveryId: number): void {
    this.loading = true;
    
    this.invoiceService.GetTransferredRefundsMainData(deliveryId).subscribe({
      next: (data) => {
  ...
  printRefund(documentNumber: string, invoiceNumber: string, year: string): void {
    ...
    this.reportService.GenerateTransferedRefundPDF(documentNumber, invoiceNumber, year).subscribe({
      next: (response: Blob) => {
