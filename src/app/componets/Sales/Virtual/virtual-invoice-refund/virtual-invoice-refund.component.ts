...
@Component({
  selector: 'app-virtual-invoice-refund',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './virtual-invoice-refund.component.html',
  styleUrl: './virtual-invoice-refund.component.scss'
})
export class VirtualInvoiceRefundComponent implements OnInit {
   ...
   */
  private loadRefunds(deliveryId: number = 0): void {
    debugger;
    this.loading = true;
    
    this.virtualInvoiceService.getRefunds(deliveryId).subscribe({
      next: (data) => {
  ...
  deleteRefund(): void {
    ...
    this.virtualInvoiceService.deleteRefund(documentNumber, billNumber, financialYear).subscribe({
      next: (response) => {
        this.toastr.success(
