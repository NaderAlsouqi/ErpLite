...
@Component({
  selector: 'app-virtual-refund-details',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './virtual-refund-details.component.html',
  styleUrl: './virtual-refund-details.component.scss'
})
export class VirtualRefundDetailsComponent implements OnInit {
  // Refund data
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (this.documentNumber && this.invoiceNumber && this.financialYear) {
        ...
        this.toastr.error(
          this.translate.instant('VirtualRefundDetailsPage.ParametersNotProvided'),
  ...
  private loadRefundDetails(documentNumber: string, invoiceNumber: string, year: string): void {
    ...
    this.virtualInvoiceService.getRefundDetails(documentNumber, invoiceNumber, year).subscribe({
      next: (data: any) => {
        if (data && data.Header) {
          ...
          if (data.Items && Array.isArray(data.Items)) {
            const formattedItems = data.Items.map((item: RefundItem) => ({
              ...
              UnitPrice: item.UnitPrice.toFixed(this.cs.billDecimals),
              DiscountAmount: item.DiscountAmount.toFixed(this.cs.billDecimals),
              TaxAmount: item.TaxAmount.toFixed(this.cs.billDecimals),
          ...
          this.toastr.warning(
            this.translate.instant('VirtualRefundDetailsPage.NoRefundDataFound'),
      ...
      error: (error) => {
        ...
        this.toastr.error(
          this.translate.instant('VirtualRefundDetailsPage.FailedToLoadRefundDetails'),
          this.translate.instant('General.Error')
        );
        this.loading = false;
      }
  ...
  printRefund(): void {
    ...
    this.reportService.GenerateTransferedVirtualRefundPDF(this.documentNumber, this.invoiceNumber, this.financialYear).subscribe({
      next: (response: Blob) => {
      ...
      error: (err) => {
        ...
        this.toastr.error(
          this.translate.instant('VirtualRefundDetailsPage.FailedToGeneratePDF'),
          this.translate.instant('General.Error')
        );
        this.loading = false;
      },
