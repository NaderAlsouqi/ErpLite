...
import {
  ChequeEndorsementService,
  ChequeEndorsementListItemDto,
  ChequeEndorsementLineDto,
  SaveChequeEndorsementRequest,
  SaveChequeEndorsementLine,
  AvailableChequeForEndorsementDto,
} from '../../../shared/services/cheque-endorsement.service';
...
export interface ChequeEndorsementLine {
  cheqNum: string;
  amt: number;
  draw: string;
  vhrNo: string;
...
@Component({
  selector: 'app-cheque-endorsement',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './cheque-endorsement.component.html',
  styleUrl: './cheque-endorsement.component.scss',
  encapsulation: ViewEncapsulation.None,
})
export class ChequeEndorsementComponent implements OnInit {
  ...
  availableCheques: AvailableChequeForEndorsementDto[] = [];
  ...
  dataSource = new MatTableDataSource<ChequeEndorsementListItemDto>([]);
  allListData: ChequeEndorsementListItemDto[] = [];
  totalItems = 0;
  pageSize = 20;
  pageIndex = 0;
  ...
  rowLinesCache = new Map<string, ChequeEndorsementLineDto[]>();
  expandLoadingKey: string | null = null;

  private rowKey(row: ChequeEndorsementListItemDto): string {
    return `${row.DocNum}|${row.VType}|${row.MyYear}`;
  }

  toggleRowExpand(row: ChequeEndorsementListItemDto): void {
    ...
    if (!this.rowLinesCache.has(key)) {
      ...
      this.endorsementService.getVoucher(row.DocNum, row.MyYear, row.VType).subscribe({
        next: v => {
  ...
  isRowExpanded(row: ChequeEndorsementListItemDto): boolean {
    return this.expandedRowKey === this.rowKey(row);
  }

  isRowLoading(row: ChequeEndorsementListItemDto): boolean {
    return this.expandLoadingKey === this.rowKey(row);
  }

  getExpandedLines(row: ChequeEndorsementListItemDto): ChequeEndorsementLineDto[] {
  ...
  constructor(
    ...
    private endorsementService: ChequeEndorsementService,
  ...
  loadLookups(): void {
    ...
    this.voucherSerialService.getAll(19).subscribe({
      next: d => {
    ...
    this.endorsementService.getAvailableCheques().subscribe({
  ...
  loadDocNums(): void {
    this.endorsementService.getList(this.myYear, this.vType, 1, 9999).subscribe({
      next: res => {
  ...
  initNewVoucher(): void {
    this.loading = true;
    this.endorsementService.getNextDocNum(this.myYear, this.vType).subscribe({
      next: res => {
  ...
  loadNavigation(): void {
    this.endorsementService.getNavigation(this.myYear, this.vType).subscribe({
  ...
  loadVoucher(docNum: number): void {
    this.loading = true;
    this.endorsementService.getVoucher(docNum, this.myYear, this.vType).subscribe({
      next: v => {
        const h = v.Header;
        ...
        for (const l of this.lines) {
          if (l.cheqNum && !knownNums.has(l.cheqNum)) {
            ...
            } as AvailableChequeForEndorsementDto];
  ...
  goPrev(): void {
    this.endorsementService.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'PREV').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('ChequeEndorsement.FirstVoucher'), ''),
    });
  }

  goNext(): void {
    this.endorsementService.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'NEXT').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('ChequeEndorsement.LastVoucher'), ''),
    });
  }
  ...
  removeLine(i: number): void {
    if (this.isSaved) {
      this.toastr.warning(
        this.translate.instant('ChequeEndorsement.CannotRemoveLine'),
  ...
  onCustAccSelected(i: number, acc: ChartOfAccountDto | null): void {
    this.lines[i].custAcc     = acc?.no ?? null;
    this.lines[i].custAccName = acc?.name ?? '';
  }

  fillLineFromCheque(i: number, cheq: AvailableChequeForEndorsementDto): void {
  ...
  save(): void {
    ...
    const req: SaveChequeEndorsementRequest = {
      DocNum:    this.docNum,
      VType:     this.vType,
      MyYear:    this.myYear,
      BrNo:      this.brNo,
      ...
      Lines:     this.lines
        ...
        .map<SaveChequeEndorsementLine>(l => ({
          CheqNum: l.cheqNum,
          Amt:     l.amt,
          Draw:    l.draw,
          BankNum: l.bankNum ?? 0,
    ...
    this.saving = true;
    this.endorsementService.save(req).subscribe({
      next: result => {
        this.toastr.success(
          this.translate.instant('ChequeEndorsement.SavedSuccessfully', { docNum: result.DocNum }),
  ...
  private validateForm(): boolean {
    if (this.rate < 0) {
      this.toastr.warning(
        this.translate.instant('ChequeEndorsement.RateNegative'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    if (this.date) {
      ...
      if (y !== this.myYear) {
        this.toastr.warning(
          this.translate.instant('ChequeEndorsement.DateOutOfYear', { year: this.myYear }),
          this.translate.instant('General.ValidationError')
        );
        return false;
      }
    ...
    if (!this.creditAcc) {
      this.toastr.warning(
        this.translate.instant('ChequeEndorsement.CreditAccRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (validLines.length === 0) {
      this.toastr.warning(
        this.translate.instant('ChequeEndorsement.LinesRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (keys.find((k, i) => keys.indexOf(k) !== i)) {
      this.toastr.warning(
        this.translate.instant('ChequeEndorsement.DuplicateCheq'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
  ...
  executeDelete(): void {
    this.modalRef?.close();
    this.loading = true;
    this.endorsementService.delete(this.docNum, this.myYear, this.vType).subscribe({
      next: () => {
        this.toastr.success(
          this.translate.instant('ChequeEndorsement.DeletedSuccessfully'),
          this.translate.instant('General.Success')
        );
        this.initNewVoucher();
      },
  ...
  loadList(): void {
    ...
    this.endorsementService.getList(
  ...
  openVoucherFromList(item: ChequeEndorsementListItemDto): void {
    this.activeTab = 'form';
  ...
  printVoucher(): void {
    ...
    const title = this.translate.instant('ChequeEndorsement.Title');
    ...
    const filtersHtml = [
      fi(this.translate.instant('ChequeEndorsement.DocNum'),    String(this.docNum)),
      fi(this.translate.instant('ChequeEndorsement.Year'),      String(this.myYear)),
      fi(this.translate.instant('ChequeEndorsement.Date'),      this.date),
      fi(this.translate.instant('ChequeEndorsement.CreditAcc'), `${this.creditAcc || ''} — ${this.creditAccName}`),
    ].join('');

    const cols = [
      { label: this.translate.instant('ChequeEndorsement.CheqNum') },
      { label: this.translate.instant('ChequeEndorsement.Amt') },
      { label: this.translate.instant('ChequeEndorsement.Draw') },
      { label: this.translate.instant('ChequeEndorsement.VhrNo') },
      { label: this.translate.instant('ChequeEndorsement.Date1') },
      { label: this.translate.instant('ChequeEndorsement.CustAcc') },
      { label: this.translate.instant('ChequeEndorsement.BankNum') },
      { label: this.translate.instant('ChequeEndorsement.Bank') },
    ];
  ...
  // ─── Lookup helpers ─────────────────────────────────────────
  chequeSearchFn(term: string, item: AvailableChequeForEndorsementDto): boolean {
    if (!term) return true;
