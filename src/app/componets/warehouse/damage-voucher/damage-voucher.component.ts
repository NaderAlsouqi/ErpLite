...
import { DamageVoucher, DamageLine, DamageLookups, DamageListRow, DamageGLRow, DmgCategoryItem, DamageVoucherService } from '../../../shared/services/damage-voucher.service';
...
@Component({
  ...
  templateUrl: './damage-voucher.component.html',
  styleUrls: ['./damage-voucher.component.scss'],
})
export class DamageVoucherComponent implements OnInit {
  @ViewChild('confirmModal') confirmModal!: ConfirmationModalComponent;
  @ViewChild('serialModal') serialModal!: SerialEntryModalComponent;

  // serial-number system (kind=3 damage, docType '22')
  private readonly KIND = 3;
  private readonly DOCTYPE = '22';

  activeTab: 'form' | 'list' = 'form';
  readonly currentYear = new Date().getFullYear();

  current: DamageVoucher = this.init();
  lookups: DamageLookups = { SerialTypes: [], Entities: [], Stores: [], Categories: [] };
  ...
  // vouchers list tab
  vouchers: DamageListRow[] = [];
  listFilter = '';
  listLoading = false;
  listLoaded = false;

  // GL entries (قيود السند)
  glEntries: DamageGLRow[] = [];
  glLoading = false;
  showGL = false;
  ...
  constructor(
    private svc: DamageVoucherService,
  ...
  get canViewCost(): boolean { return this.auth.hasPermission('Damage.ViewCost'); }
  ...
  onLineCreditAccChange(line: DamageLine): void { line.CreditAccName = this.accName(line.CreditAcc); }
  ...
  get filteredVouchers(): DamageListRow[] {
  ...
  openVoucher(row: DamageListRow): void {
  ...
  load(): void {
    if (!this.current.VType) { this.toastr.warning(this.translate.instant('Damage.SelectSerial')); return; }
    if (!this.current.DocNo) { this.toastr.warning(this.translate.instant('Damage.EnterDocNo')); return; }
    this.svc.get(this.current.VType, this.current.DocNo, this.current.Myear!).subscribe({
      next: (v) => {
        ...
        this.loadSerials();
        this.toastr.success(this.translate.instant('Damage.Loaded'));
      },
      error: (err) => {
        if (err.status === 404) this.toastr.info(this.translate.instant('Damage.NotFound'));
  ...
  // ─── detail grid ──────────────────────────────────────────────
  onItemPick(line: DamageLine): void {
  ...
  onUnitChange(line: DamageLine): void {
  ...
  doBring(): void {
    if (!this.bringCategory) { this.toastr.warning(this.translate.instant('Damage.SelectCategory')); return; }
    this.svc.itemsByCategory(this.bringCategory).subscribe({
      next: (items: DmgCategoryItem[]) => {
        if (!items.length) { this.toastr.info(this.translate.instant('Damage.NoItemsInCategory')); return; }
        ...
        this.toastr.success(this.translate.instant('Damage.Brought'));
      },
  ...
  showGLEntries(): void {
    if (!this.isExisting || !this.current.VType || !this.current.DocNo) {
      this.toastr.info(this.translate.instant('Damage.SaveFirstGL'));
      return;
    }
    this.glLoading = true;
    this.glEntries = [];
  ...
  onStoreChange(line: DamageLine): void {
  ...
  loadItemStock(line: DamageLine): void {
  ...
  setMaxQty(line: DamageLine): void {
  ...
  onQtyChange(line: DamageLine): void {
    if (line.MaxQty != null && +(line.Qty || 0) > line.MaxQty) {
      line.Qty = line.MaxQty;
      this.toastr.warning(this.translate.instant('Damage.QtyExceedsStock', { n: line.MaxQty }));
    }
    this.recalc(line);
    this.updateFifoCost(line);
  }
  ...
  baseQty(line: DamageLine): number { return Math.round((+(line.Qty || 0)) * (+(line.UnitRate || 1) || 1)); }

  /** Open the serial picker for a line (damage → pick in-stock serials). */
  async openSerials(line: DamageLine): Promise<void> {
    if (!(line.ItemNo || '').trim()) { this.toastr.warning(this.translate.instant('Damage.PickItemFirst')); return; }
    if (!line.StoreNo) { this.toastr.warning(this.translate.instant('Damage.StoreRequired')); return; }
    const need = this.baseQty(line);
    if (need <= 0) { this.toastr.warning(this.translate.instant('Damage.QtyRequired')); return; }
  ...
  private savableLines(): DamageLine[] {
    return this.current.Lines.filter(l => (l.ItemNo || '').trim() && +(l.Qty || 0) > 0);
  }

  private validate(): boolean {
    if (!this.current.VType) { this.toastr.warning(this.translate.instant('Damage.SelectSerial')); return false; }
    if (!this.current.DocNo?.trim()) { this.toastr.warning(this.translate.instant('Damage.EnterDocNo')); return false; }
    ...
    if (!itemLines.length) { this.toastr.warning(this.translate.instant('Damage.NoLines')); return false; }
    if (itemLines.some(l => !(+(l.Qty || 0) > 0))) { this.toastr.warning(this.translate.instant('Damage.QtyRequired')); return false; }
    if (itemLines.some(l => !l.StoreNo)) { this.toastr.warning(this.translate.instant('Damage.StoreRequired')); return false; }
    if (itemLines.some(l => !l.UnitNo)) { this.toastr.warning(this.translate.instant('Damage.UnitRequired')); return false; }
  ...
  printList(): void {
    ...
    const cols = [
      { label: t('Damage.DocNo') }, { label: t('Damage.Serial') }, { label: t('Damage.Date') },
      { label: t('Damage.Entity') },
      ...(showCost ? [{ label: t('Damage.GrandTotal') }] : []), { label: t('Damage.LinesCount') },
    ];
    ...
    const totRow = showCost
      ? `<tr style="font-weight:700;background:#eef2ff"><td colspan="4" style="text-align:end">${t('Damage.GrandTotal')}</td><td>${num(grand)}</td><td></td></tr>`
      : '';
    const filtersHtml =
      `<div class="filter-item"><span class="filter-label">${t('Damage.Year')}:</span><span class="filter-value">${this.current.Myear ?? ''}</span></div>`;
    this.reportService.printReport(t('Damage.ListTab'), cols, rows + totRow, filtersHtml);
  }
  ...
  print(): void {
    ...
    const showCost = this.canViewCost;
    const cols = [
      { label: t('Damage.ItemCode') }, { label: t('Damage.ItemName') }, { label: t('Damage.Unit') },
      { label: t('Damage.Store') }, { label: t('Damage.Qty') },
      ...(showCost ? [{ label: t('Damage.Cost') }, { label: t('Damage.Total') }] : []),
      { label: t('Damage.Expiry') }, { label: t('Damage.Batch') },
    ];
    ...
    const totRow = showCost
      ? `<tr style="font-weight:700;background:#eef2ff"><td colspan="6" style="text-align:end">${t('Damage.GrandTotal')}</td><td>${num(this.grandTotal)}</td><td colspan="2"></td></tr>`
      : '';
    ...
    const filtersHtml =
      fi(t('Damage.DocNo'), this.current.DocNo) +
      fi(t('Damage.Serial'), this.current.VTypeName) +
      fi(t('Damage.Date'), this.current.TransDate) +
      fi(t('Damage.Entity'), this.current.TgName) +
      fi(t('Damage.Year'), this.current.Myear) +
      fi(t('Damage.Notes'), this.current.Des);
    const footerHtml = showCost
      ? `<b>${t('Damage.TotalWeight')}:</b> ${num(this.totalWeight)} &nbsp;&nbsp;&nbsp; <b>${t('Damage.GrandTotal')}:</b> ${num(this.grandTotal)}`
      : `<b>${t('Damage.TotalWeight')}:</b> ${num(this.totalWeight)}`;

    this.reportService.printReport(t('Damage.Title'), cols, rows + totRow, filtersHtml, footerHtml);
  }
}
