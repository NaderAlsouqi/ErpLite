...
import {
  ChequeWithdrawalService,
  ChequeWithdrawalListItemDto,
  ChequeWithdrawalLineDto,
  SaveChequeWithdrawalRequest,
  SaveChequeWithdrawalLine,
  AvailableChequeForWithdrawalDto,
} from '../../../shared/services/cheque-withdrawal.service';
...
export interface ChequeWithdrawalLine {
  cheqNum: string;
  amt: number;
  draw: string;
  vhrNo: string;
...
@Component({
  ...
  styleUrl: './cheque-withdrawal.component.scss',
...
export class ChequeWithdrawalComponent implements OnInit {
  ...
  // ─── Available cheques (status IN 1,2 → after save: status=2 محصل) ──
  availableCheques: AvailableChequeForWithdrawalDto[] = [];
  ...
  dataSource = new MatTableDataSource<ChequeWithdrawalListItemDto>([]);
  allListData: ChequeWithdrawalListItemDto[] = [];
  totalItems = 0;
  pageSize = 20;
  pageIndex = 0;
  ...
  rowLinesCache = new Map<string, ChequeWithdrawalLineDto[]>();
  expandLoadingKey: string | null = null;

  private rowKey(row: ChequeWithdrawalListItemDto): string {
    return `${row.DocNum}|${row.VType}|${row.MyYear}`;
  }

  toggleRowExpand(row: ChequeWithdrawalListItemDto): void {
    ...
    if (!this.rowLinesCache.has(key)) {
      ...
      this.chequeWithdrawalService.getVoucher(row.DocNum, row.MyYear, row.VType).subscribe({
        next: v => {
  ...
  isRowExpanded(row: ChequeWithdrawalListItemDto): boolean {
    return this.expandedRowKey === this.rowKey(row);
  }

  isRowLoading(row: ChequeWithdrawalListItemDto): boolean {
    return this.expandLoadingKey === this.rowKey(row);
  }

  getExpandedLines(row: ChequeWithdrawalListItemDto): ChequeWithdrawalLineDto[] {
  ...
  constructor(
    ...
    private chequeWithdrawalService: ChequeWithdrawalService,
  ...
  loadLookups(): void {
    ...
    this.voucherSerialService.getAll(12).subscribe({
      next: d => {
    ...
    this.chequeWithdrawalService.getAllCheques().subscribe({
  ...
  loadDocNums(): void {
    this.chequeWithdrawalService.getList(this.myYear, this.vType, 1, 9999).subscribe({
      next: res => {
  ...
  initNewVoucher(): void {
    this.loading = true;
    this.chequeWithdrawalService.getNextDocNum(this.myYear, this.vType).subscribe({
      next: res => {
  ...
  loadNavigation(): void {
    this.chequeWithdrawalService.getNavigation(this.myYear, this.vType).subscribe({
  ...
  loadVoucher(docNum: number): void {
    this.loading = true;
    this.chequeWithdrawalService.getVoucher(docNum, this.myYear, this.vType).subscribe({
      next: v => {
        const h = v.Header;
        ...
        for (const l of this.lines) {
          if (l.cheqNum && !knownNums.has(l.cheqNum)) {
            ...
            } as AvailableChequeForWithdrawalDto];
  ...
  goPrev(): void {
    this.chequeWithdrawalService.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'PREV').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('ChequeWithdrawal.FirstVoucher'), ''),
    });
  }

  goNext(): void {
    this.chequeWithdrawalService.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'NEXT').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('ChequeWithdrawal.LastVoucher'), ''),
    });
  }
  ...
  removeLine(i: number): void {
    if (this.isSaved) {
      this.toastr.warning(
        this.translate.instant('ChequeWithdrawal.CannotRemoveLine'),
  ...
  // ─── Cheque dropdown open: reload all cheques (no filter) ───
  onChequeDropdownOpen(): void {
    this.chequeWithdrawalService.getAllCheques().subscribe({
      next: d => { this.availableCheques = d; },
      error: () => {}
    });
  ...
  fillLineFromCheque(i: number, cheq: AvailableChequeForWithdrawalDto): void {
  ...
  save(): void {
    ...
    const req: SaveChequeWithdrawalRequest = {
      DocNum:    this.docNum,
      VType:     this.vType,
      MyYear:    this.myYear,
      BrNo:      this.brNo,
      ...
      Lines:     this.lines
        ...
        .map<SaveChequeWithdrawalLine>(l => ({
          CheqNum: l.cheqNum,
          Amt:     l.amt,
          Draw:    l.draw,
          BankNum: l.bankNum ?? 0,
    ...
    this.saving = true;
    this.chequeWithdrawalService.save(req).subscribe({
      next: result => {
        this.toastr.success(
          this.translate.instant('ChequeWithdrawal.SavedSuccessfully', { docNum: result.DocNum }),
  ...
  private validateForm(): boolean {
    if (this.rate < 0) {
      this.toastr.warning(
        this.translate.instant('ChequeWithdrawal.RateNegative'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    if (this.date) {
      ...
      if (y !== this.myYear) {
        this.toastr.warning(
          this.translate.instant('ChequeWithdrawal.DateOutOfYear', { year: this.myYear }),
          this.translate.instant('General.ValidationError')
        );
        return false;
      }
    ...
    if (!this.creditAcc) {
      this.toastr.warning(
        this.translate.instant('ChequeWithdrawal.CreditAccRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (validLines.length === 0) {
      this.toastr.warning(
        this.translate.instant('ChequeWithdrawal.LinesRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (keys.find((k, i) => keys.indexOf(k) !== i)) {
      this.toastr.warning(
        this.translate.instant('ChequeWithdrawal.DuplicateCheq'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
  ...
  executeDelete(): void {
    this.modalRef?.close();
    this.loading = true;
    this.chequeWithdrawalService.delete(this.docNum, this.myYear, this.vType).subscribe({
      next: () => {
        this.toastr.success(
          this.translate.instant('ChequeWithdrawal.DeletedSuccessfully'),
          this.translate.instant('General.Success')
        );
        this.initNewVoucher();
      },
  ...
  loadList(): void {
    ...
    this.chequeWithdrawalService.getList(
  ...
  openVoucherFromList(item: ChequeWithdrawalListItemDto): void {
    this.activeTab = 'form';
  ...
  printVoucher(): void {
    ...
    const title = this.translate.instant('ChequeWithdrawal.Title');
    ...
    const filtersHtml = [
      fi(this.translate.instant('ChequeWithdrawal.DocNum'),    String(this.docNum)),
      fi(this.translate.instant('ChequeWithdrawal.Year'),      String(this.myYear)),
      fi(this.translate.instant('ChequeWithdrawal.Date'),      this.date),
      fi(this.translate.instant('ChequeWithdrawal.CreditAcc'), `${this.creditAcc || ''} — ${this.creditAccName}`),
    ].join('');

    const cols = [
      { label: this.translate.instant('ChequeWithdrawal.CheqNum') },
      { label: this.translate.instant('ChequeWithdrawal.Amt') },
      { label: this.translate.instant('ChequeWithdrawal.Draw') },
      { label: this.translate.instant('ChequeWithdrawal.VhrNo') },
      { label: this.translate.instant('ChequeWithdrawal.Date1') },
      { label: this.translate.instant('ChequeWithdrawal.CustAcc') },
      { label: this.translate.instant('ChequeWithdrawal.BankNum') },
      { label: this.translate.instant('ChequeWithdrawal.Bank') },
    ];
