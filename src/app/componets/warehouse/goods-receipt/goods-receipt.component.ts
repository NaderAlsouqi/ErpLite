...
import { GoodsReceipt, GoodsReceiptLine, GoodsReceiptLookups, GoodsReceiptListRow, GoodsReceiptGLRow, GrCategoryItem, GoodsReceiptService } from '../../../shared/services/goods-receipt.service';
...
export class GoodsReceiptComponent implements OnInit {
  ...
  private readonly DOCTYPE = '37';

  activeTab: 'form' | 'list' = 'form';
  readonly currentYear = new Date().getFullYear();

  current: GoodsReceipt = this.init();
  lookups: GoodsReceiptLookups = { SerialTypes: [], Vendors: [], Entities: [], Stores: [], Categories: [], PurchaseOrders: [] };
  ...
  // vouchers list tab
  vouchers: GoodsReceiptListRow[] = [];
  listFilter = '';
  listLoading = false;
  listLoaded = false;

  // GL entries (قيود السند)
  glEntries: GoodsReceiptGLRow[] = [];
  glLoading = false;
  showGL = false;
  ...
  get parentTitleKey(): string {
    return 'Nav.Purchases.Documents';   // lives under نظام ادارة المشتريات → المستندات
  }
  ...
  ngOnInit(): void {
    this.permPrefix = this.route.snapshot.data['permPrefix'] || 'GoodsReceipt';
  ...
  onLineDebitAccChange(line: GoodsReceiptLine): void { line.DebitAccName = this.accName(line.DebitAcc); }
  onLineCreditAccChange(line: GoodsReceiptLine): void { line.CreditAccName = this.accName(line.CreditAcc); }
  ...
  private applySerialAccDefaults(line: GoodsReceiptLine): void {
  ...
  get filteredVouchers(): GoodsReceiptListRow[] {
  ...
  openVoucher(row: GoodsReceiptListRow): void {
  ...
  private init(): GoodsReceipt {
    return {
      ...
      RcptPONo: '', RcptPOYear: null, RcptPOVType: null,
      Lines: [this.newLine()],
    };
  }

  // ─── أمر الشراء link ──────────────────────────────────────────
  /** Picking a PO carries its vendor over; the ⬇ button pulls its lines. */
  onPoChange(): void {
    const po = this.lookups?.PurchaseOrders?.find((x: any) => x.PONo === this.current.RcptPONo);
    if (!po) { this.current.RcptPOYear = null; this.current.RcptPOVType = null; return; }
    this.current.RcptPOYear = po.Myear;
    this.current.RcptPOVType = 0;
    if (!this.current.VendorNo && po.VenNo) { this.current.VendorNo = po.VenNo; this.onVendorChange(); }
  }

  /** Load the أمر الشراء's outstanding lines into the grid (defaulting the
   *  received qty to what is still remaining). */
  loadFromPO(): void {
    const no = (this.current.RcptPONo || '').toString().trim();
    if (!no) { this.toastr.warning(this.translate.instant('GoodsReceipt.PickPoFirst')); return; }
    this.svc.fromPO(no, this.current.RcptPOYear || (this.current.Myear as number)).subscribe({
      next: (rows) => {
        if (!rows || !rows.length) { this.toastr.info(this.translate.instant('GoodsReceipt.PoEmpty')); return; }
        const mapped: GoodsReceiptLine[] = rows.map((r) => ({
          ...this.newLine(),
          ItemNo: r.ItemNo, ItemName: r.ItemName, Ename: r.Ename,
          UnitNo: r.UnitNo, UnitName: r.UnitName, UnitRate: r.UnitRate || 1,
          StoreNo: r.StoreNo || this.defaultStore(), Qty: r.RemainingQty, Cost: r.Cost ?? null,
          Expired: !!r.Expired,
          OrderedQty: r.OrderedQty, ReceivedQty: r.ReceivedQty, RemainingQty: r.RemainingQty,
        }));
        const existing = this.savableLines();
        this.current.Lines = existing.length ? [...existing, ...mapped] : mapped;
        // pull each item's units + apply account defaults, and recalc line totals
        this.current.Lines.forEach((l) => { if (l.ItemNo) this.onItemPick(l); });
        this.toastr.success(this.translate.instant('GoodsReceipt.PoLoaded', { n: mapped.length }));
      },
  ...
  load(): void {
    if (!this.current.VType) { this.toastr.warning(this.translate.instant('GoodsReceipt.SelectSerial')); return; }
    if (!this.current.DocNo) { this.toastr.warning(this.translate.instant('GoodsReceipt.EnterDocNo')); return; }
    this.svc.get(this.current.VType, this.current.DocNo, this.current.Myear!).subscribe({
      next: (v) => {
        ...
        this.loadSerials();
        this.toastr.success(this.translate.instant('GoodsReceipt.Loaded'));
      },
      error: (err) => {
        if (err.status === 404) this.toastr.info(this.translate.instant('GoodsReceipt.NotFound'));
  ...
  // ─── detail grid ──────────────────────────────────────────────
  onItemPick(line: GoodsReceiptLine): void {
  ...
  onUnitChange(line: GoodsReceiptLine): void {
  ...
  doBring(): void {
    if (!this.bringCategory) { this.toastr.warning(this.translate.instant('GoodsReceipt.SelectCategory')); return; }
    this.svc.itemsByCategory(this.bringCategory).subscribe({
      next: (items: GrCategoryItem[]) => {
        if (!items.length) { this.toastr.info(this.translate.instant('GoodsReceipt.NoItemsInCategory')); return; }
        ...
        this.toastr.success(this.translate.instant('GoodsReceipt.Brought'));
      },
      error: (err) => this.toastr.error(err.error?.message || this.translate.instant('General.Error'))
    });
  }
  ...
  showGLEntries(): void {
    if (!this.isExisting || !this.current.VType || !this.current.DocNo) {
      this.toastr.info(this.translate.instant('GoodsReceipt.SaveFirstGL'));
      return;
    }
    this.glLoading = true;
    this.glEntries = [];
  ...
  onStoreChange(line: GoodsReceiptLine): void {
  ...
  recalc(line: GoodsReceiptLine): void {
  ...
  baseQty(line: GoodsReceiptLine): number { return Math.round((+(line.Qty || 0)) * (+(line.UnitRate || 1) || 1)); }

  /** Open the serial entry for a line (inbound → type new serials). */
  async openSerials(line: GoodsReceiptLine): Promise<void> {
    if (!(line.ItemNo || '').trim()) { this.toastr.warning(this.translate.instant('GoodsReceipt.PickItemFirst')); return; }
    if (!line.StoreNo) { this.toastr.warning(this.translate.instant('GoodsReceipt.StoreRequired')); return; }
    const need = this.baseQty(line);
    if (need <= 0) { this.toastr.warning(this.translate.instant('GoodsReceipt.QtyRequired')); return; }
  ...
  private savableLines(): GoodsReceiptLine[] {
    return this.current.Lines.filter(l => (l.ItemNo || '').trim() && +(l.Qty || 0) > 0);
  }

  private validate(): boolean {
    if (!this.current.VType) { this.toastr.warning(this.translate.instant('GoodsReceipt.SelectSerial')); return false; }
    if (!this.current.DocNo?.trim()) { this.toastr.warning(this.translate.instant('GoodsReceipt.EnterDocNo')); return false; }
    ...
    if (!itemLines.length) { this.toastr.warning(this.translate.instant('GoodsReceipt.NoLines')); return false; }
    if (itemLines.some(l => !(+(l.Qty || 0) > 0))) { this.toastr.warning(this.translate.instant('GoodsReceipt.QtyRequired')); return false; }
    if (itemLines.some(l => !l.StoreNo)) { this.toastr.warning(this.translate.instant('GoodsReceipt.StoreRequired')); return false; }
    if (itemLines.some(l => !l.UnitNo)) { this.toastr.warning(this.translate.instant('GoodsReceipt.UnitRequired')); return false; }
  ...
  async save(): Promise<void> {
    ...
    this.svc.save(payload).subscribe({
      next: (res) => {
        ...
        if (res.inbDocNo) this.current.InbDocNo = res.inbDocNo;
        this.isExisting = true;
        this.listLoaded = false;   // list may be stale
        this.postSerials(res.docNo);
        // the receipt internally created a سند إدخال مواد — surface its number
        this.toastr.success(res.inbDocNo
          ? this.translate.instant('GoodsReceipt.SavedWithInbound', { no: res.inbDocNo })
          : (res.message || this.translate.instant('General.SaveSuccess')));
      },
  ...
  printList(): void {
    ...
    const cols = [
      { label: t('GoodsReceipt.DocNo') }, { label: t('GoodsReceipt.Serial') }, { label: t('GoodsReceipt.Date') },
      { label: t('GoodsReceipt.Vendor') }, { label: t('GoodsReceipt.Entity') },
      ...(showCost ? [{ label: t('GoodsReceipt.GrandTotal') }] : []), { label: t('GoodsReceipt.LinesCount') },
    ];
    ...
    const totRow = showCost
      ? `<tr style="font-weight:700;background:#eef2ff"><td colspan="5" style="text-align:end">${t('GoodsReceipt.GrandTotal')}</td><td>${num(grand)}</td><td></td></tr>`
      : '';
    const filtersHtml =
      `<div class="filter-item"><span class="filter-label">${t('GoodsReceipt.Year')}:</span><span class="filter-value">${this.current.Myear ?? ''}</span></div>`;
    this.reportService.printReport(t('GoodsReceipt.ListTab'), cols, rows + totRow, filtersHtml);
  }
  ...
  print(): void {
    ...
    const showCost = this.canViewCost;
    const cols = [
      { label: t('GoodsReceipt.ItemCode') }, { label: t('GoodsReceipt.ItemName') }, { label: t('GoodsReceipt.Unit') },
      { label: t('GoodsReceipt.Store') }, { label: t('GoodsReceipt.Qty') },
      ...(showCost ? [{ label: t('GoodsReceipt.Cost') }, { label: t('GoodsReceipt.Total') }] : []),
      { label: t('GoodsReceipt.Expiry') }, { label: t('GoodsReceipt.Batch') },
    ];
    ...
    const totRow = showCost
      ? `<tr style="font-weight:700;background:#eef2ff"><td colspan="6" style="text-align:end">${t('GoodsReceipt.GrandTotal')}</td><td>${num(this.grandTotal)}</td><td colspan="2"></td></tr>`
      : '';
    ...
    const filtersHtml =
      fi(t('GoodsReceipt.DocNo'), this.current.DocNo) +
      fi(t('GoodsReceipt.Serial'), this.current.VTypeName) +
      fi(t('GoodsReceipt.Date'), this.current.TransDate) +
      fi(t('GoodsReceipt.Vendor'), this.current.VendorName) +
      fi(t('GoodsReceipt.Year'), this.current.Myear) +
      fi(t('GoodsReceipt.Notes'), this.current.Des);
    const footerHtml = showCost
      ? `<b>${t('GoodsReceipt.TotalWeight')}:</b> ${num(this.totalWeight)} &nbsp;&nbsp;&nbsp; <b>${t('GoodsReceipt.GrandTotal')}:</b> ${num(this.grandTotal)}`
      : `<b>${t('GoodsReceipt.TotalWeight')}:</b> ${num(this.totalWeight)}`;

    this.reportService.printReport(t('GoodsReceipt.Title'), cols, rows + totRow, filtersHtml, footerHtml);
  }
}
