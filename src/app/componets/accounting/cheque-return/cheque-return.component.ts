...
import {
  ChequeReturnService,
  ChequeReturnListItemDto,
  ChequeReturnLineDto,
  SaveChequeReturnRequest,
  SaveChequeReturnLine,
  AvailableChequeForReturnDto,
} from '../../../shared/services/cheque-return.service';
...
@Component({
  ...
  styleUrl: './cheque-return.component.scss',
...
export class ChequeReturnComponent implements OnInit {
  ...
  // ─── Available cheques (status=2 محصل) ───────────────────────
  availableCheques: AvailableChequeForReturnDto[] = [];
  ...
  dataSource = new MatTableDataSource<ChequeReturnListItemDto>([]);
  allListData: ChequeReturnListItemDto[] = [];
  totalItems = 0;
  pageSize = 20;
  pageIndex = 0;
  ...
  rowLinesCache = new Map<string, ChequeReturnLineDto[]>();
  expandLoadingKey: string | null = null;

  private rowKey(row: ChequeReturnListItemDto): string {
    return `${row.DocNum}|${row.VType}|${row.MyYear}`;
  }
  toggleRowExpand(row: ChequeReturnListItemDto): void {
    ...
    if (!this.rowLinesCache.has(key)) {
      ...
      this.returnService.getVoucher(row.DocNum, row.MyYear, row.VType).subscribe({
        next: v => {
  ...
  isRowExpanded(row: ChequeReturnListItemDto): boolean { return this.expandedRowKey === this.rowKey(row); }
  isRowLoading(row: ChequeReturnListItemDto): boolean { return this.expandLoadingKey === this.rowKey(row); }
  getExpandedLines(row: ChequeReturnListItemDto): ChequeReturnLineDto[] {
  ...
  constructor(
    ...
    private returnService: ChequeReturnService,
  ...
  loadLookups(): void {
    ...
    this.voucherSerialService.getAll(10).subscribe({
      next: d => {
    ...
    this.returnService.getAvailableCheques().subscribe({
  ...
  loadDocNums(): void {
    this.returnService.getList(this.myYear, this.vType, 1, 9999).subscribe({
      next: res => {
  ...
  initNewVoucher(): void {
    this.loading = true;
    this.returnService.getNextDocNum(this.myYear, this.vType).subscribe({
      next: res => {
  ...
  loadNavigation(): void {
    this.returnService.getNavigation(this.myYear, this.vType).subscribe({
  ...
  loadVoucher(docNum: number): void {
    this.loading = true;
    this.returnService.getVoucher(docNum, this.myYear, this.vType).subscribe({
      next: v => {
        const h = v.Header;
        ...
        for (const l of this.lines) {
          if (l.cheqNum && !knownNums.has(l.cheqNum)) {
            ...
            } as AvailableChequeForReturnDto];
  ...
  goPrev(): void {
    this.returnService.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'PREV').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('ChequeReturn.FirstVoucher'), ''),
    });
  }

  goNext(): void {
    this.returnService.getAdjacentDocNum(this.docNum, this.myYear, this.vType, 'NEXT').subscribe({
      next: r => this.loadVoucher(r.DocNum),
      error: () => this.toastr.info(this.translate.instant('ChequeReturn.LastVoucher'), ''),
    });
  }
  ...
  removeLine(i: number): void {
    if (this.isSaved) {
      this.toastr.warning(
        this.translate.instant('ChequeReturn.CannotRemoveLine'),
  ...
  fillLineFromCheque(i: number, cheq: AvailableChequeForReturnDto): void {
  ...
  save(): void {
    ...
    const req: SaveChequeReturnRequest = {
      DocNum:    this.docNum,
      VType:     this.vType,
      MyYear:    this.myYear,
      BrNo:      this.brNo,
    ...
    this.saving = true;
    this.returnService.save(req).subscribe({
      next: result => {
        this.toastr.success(
          this.translate.instant('ChequeReturn.SavedSuccessfully', { docNum: result.DocNum }),
  ...
  private validateForm(): boolean {
    if (this.rate < 0) {
      this.toastr.warning(
        this.translate.instant('ChequeReturn.RateNegative'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    if (this.date) {
      ...
      if (y !== this.myYear) {
        this.toastr.warning(
          this.translate.instant('ChequeReturn.DateOutOfYear', { year: this.myYear }),
          this.translate.instant('General.ValidationError')
        );
        return false;
      }
    ...
    if (!this.creditAcc) {
      this.toastr.warning(
        this.translate.instant('ChequeReturn.CreditAccRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (validLines.length === 0) {
      this.toastr.warning(
        this.translate.instant('ChequeReturn.LinesRequired'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
    ...
    if (keys.find((k, i) => keys.indexOf(k) !== i)) {
      this.toastr.warning(
        this.translate.instant('ChequeReturn.DuplicateCheq'),
        this.translate.instant('General.ValidationError')
      );
      return false;
    }
  ...
  executeDelete(): void {
    this.modalRef?.close();
    this.loading = true;
    this.returnService.delete(this.docNum, this.myYear, this.vType).subscribe({
      next: () => {
        this.toastr.success(
          this.translate.instant('ChequeReturn.DeletedSuccessfully'),
          this.translate.instant('General.Success')
        );
        this.initNewVoucher();
      },
  ...
  openVoucherFromList(item: ChequeReturnListItemDto): void {
    this.activeTab = 'form';
  ...
  printVoucher(): void {
    ...
    const title = this.translate.instant('ChequeReturn.Title');
    ...
    const filtersHtml = [
      fi(this.translate.instant('ChequeReturn.DocNum'),    String(this.docNum)),
      fi(this.translate.instant('ChequeReturn.Year'),      String(this.myYear)),
      fi(this.translate.instant('ChequeReturn.Date'),      this.date),
      fi(this.translate.instant('ChequeReturn.CreditAcc'), `${this.creditAcc || ''} — ${this.creditAccName}`),
    ].join('');

    const cols = [
      { label: this.translate.instant('ChequeReturn.CheqNum') },
      { label: this.translate.instant('ChequeReturn.Amt') },
      { label: this.translate.instant('ChequeReturn.Draw') },
      { label: this.translate.instant('ChequeReturn.VhrNo') },
      { label: this.translate.instant('ChequeReturn.Date1') },
      { label: this.translate.instant('ChequeReturn.CustAcc') },
      { label: this.translate.instant('ChequeReturn.BankNum') },
      { label: this.translate.instant('ChequeReturn.Bank') },
    ];
  ...
  // ─── Lookup helpers ─────────────────────────────────────────
  chequeSearchFn(term: string, item: AvailableChequeForReturnDto): boolean {
    if (!term) return true;
